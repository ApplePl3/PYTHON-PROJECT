...
# Lớp Event dùng để quản lý sự kiện
class Event(object):
    def __init__(self):
...
#Bắt đầu sự kiện khi người chơi bị giết.
...
#Bắt đầu sự kiện khi người chơi chiến thắng.
...
#Cập nhật trạng thái của sự kiện, xử lý logic khi sự kiện xảy ra.
            ...
            if not self.player_in_castle:
                ...
                if core.get_map().time > 0:
                    ...
                    if self.time == 0:
                        ...
                        core.get_mm().currentGameState = 'Loading'
                        core.get_mm().oLoadingMenu.set_text_and_type('Made By Thai', False)
                        core.get_mm().oLoadingMenu.update_time()
                        core.get_sound().play('game_over', 0, 0.5)
