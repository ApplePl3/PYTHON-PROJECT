...
#Class đại diện cho mọi hành động của người chơi
class Player(object):
...
#Cập nhật trạng thái của người chơi dựa trên sự kiện từ lõi (core) của trò chơi. Phương thức này thực hiện các bước vật lý, cập nhật hình ảnh của người chơi và thời gian không thể giết được.
    def update(self, core):
        self.player_physics(core)
        self.update_image(core)
        self.update_unkillable_time()

#vật lý trong game với người chơi
    def player_physics(self, core):
        ...
        # bắn cầu lửa và di chuyển nhanh 
        self.fast_moving = False
        if core.keyShift:
        ...
        if not self.on_ground:
            # nhảy lên, sử dụng mũi tên đi lên
            if (self.y_vel < 0 and core.keyU):
                self.y_vel += GRAVITY
                
            # Nhảy xuống khi người chơi từ trên cao
        ...
        # đoạn mã dùng để ổn định tham số "on_ground"
    ...
    def update_image(self, core):
        ...
        if self.powerLVL in (0, 1, 2):
            ...
            # Người chơi quyết định di chuyển sang hướng khác, nhưng vẫn chưa dừng lại
...
#Cập nhật thời gian không thể giết được của người chơi.
...
#Cập nhật vị trí x của người chơi dựa trên va chạm với các khối.
...
#Cập nhật vị trí y của người chơi dựa trên va chạm với các khối và thực hiện các hành động tương ứng.
...
#Kích hoạt hành động của khối khi người chơi va chạm với nó.
...
#người chơi thay đổi level (ăn nấm, hoa)
...
#Bắn viên đạn lửa từ người chơi.
